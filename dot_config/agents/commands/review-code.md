...
<INSTRUCTIONS>
1. **Execute the required command** to retrieve the changes:
  - First, check current changes are existed : `git diff HEAD`
  - if changes are existed, review the changes. and don't review other commits.
  - if changes are not existed, then check commits.
  - usually, base branch is develop. however, stacked changes can be applied, then base branch can be other branch.
  - **check diff between current branch and base branch**
  - if new commits existed in this branch, review new commits.
