...
- 코드 작성 시 한 줄 최대 길이: **150자** (문서 작성 시 아님)

## Flutter/Dart 프로젝트 전용 규칙

> 아래 규칙은 **프로젝트 루트에 `pubspec.yaml`이 존재하는 Flutter/Dart 프로젝트에서만** 적용한다.
> 그 외 프로젝트에서는 무시할 것.
...
세션 시작 시 `KNOWLEDGE_GRAPH_PATH` 환경변수가 설정되어 있으면:

1. `$KNOWLEDGE_GRAPH_PATH/INDEX.md`를 Read
2. 현재 프로젝트 식별 (git remote URL → 디렉토리명 폴백)
3. `$KNOWLEDGE_GRAPH_PATH/projects/{프로젝트명}/_index.md`가 있으면 Read
4. 로드 결과를 `🧠 Knowledge Graph 로드 완료 — 프로젝트: {이름}, 지식 N개` 형식으로 출력
5. 로드한 지식 목록이 비어있으면 출력하지 않음

깊은 참조가 필요할 때는 `/knowledge-graph` 스킬을 사용한다.
