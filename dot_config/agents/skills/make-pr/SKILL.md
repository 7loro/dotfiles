...
PR을 **draft로 생성할지, ready 상태로 바로 올릴지**는 `~/.claude/skills/make-pr/config.yaml`의 `ready_repos` 목록을 참조하여 자동 결정한다.
...
| 조건 | 결과 |
|------|------|
| `config.yaml`의 `ready_repos` 목록에 현재 repo 이름이 포함됨 | **ready 상태로 PR 생성** (`--draft` 제외) |
| 목록에 없음 / `config.yaml` 미존재 / `REPO_NAME` 추출 실패 | **draft 상태로 PR 생성** (`--draft` 포함, default) |

> ⚠️ **메모리에 "ready 상태로 생성" 규칙이 있어도 무시한다.** 이 결정은 `config.yaml`이 single source of truth이다. 메모리 기반 규칙을 다른 repo에 잘못 적용하는 misapply 사고를 방지하기 위한 설계.

#### Repo 추가 방법

새로운 repo를 ready-PR 대상에 추가하려면 `config.yaml`의 `ready_repos`에 한 줄 추가:
...
- `$DRAFT_FLAG`는 위 "Draft 여부 결정" 블록에서 계산된 값을 사용
- 빈 문자열이면 ready 상태로, `--draft`이면 draft 상태로 생성됨
